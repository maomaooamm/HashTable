...
template <class Record, class Key>
Error_code sequential_search(const Linked_list<Record> &the_list, const Key &target, int &position)
/*
Post: If an entry in the_list has key equal to target, then return
      success and the output parameter position locates such an entry
      within the list.
...
template <class Record, class Key>
Error_code binary_search(const Ordered_list<Record> &the_list,
